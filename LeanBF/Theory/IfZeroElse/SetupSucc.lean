...
/-!
# ifZeroElse Setup: Successor

The shared setup prefix of `Compiler.ifZeroElse` reaches the `elseBodyState`
when the tested cell is non-zero, and the setup/branch/restore runs chain
into the `ifZeroElse` post state.
