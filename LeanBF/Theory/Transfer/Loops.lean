...
The loops that move values between registers: draining one into another,
scaling on the way, and dividing by consuming several units per unit
produced.
...
Each theorem takes the program as a parameter with hypotheses pinning the
instruction slots it occupies, so a fragment can be placed anywhere inside a
larger program.

`div_reaches` resolves the incompleteness of division by encoding the
remainder in where it stops: the exit address is `exitBase + n % k`. One fragment
therefore answers both the quotient and whether the divisor divided at all.

## Main definitions

* `decState`: The drain loop's state after its `jzdec` step.
* `midState`: The drain loop's state after its `inc` step.
* `drained`: The state after the drain loop has run to completion.
* `bumped`: A register raised by a constant.
* `scaled`: The state after a scaled transfer has run to completion.
* `afterDec`: One scaled iteration's state after decrementing the source.
