...
| **2CM universality bridge** | High | The remaining capstone input, being built on the `counter-machine-bridge` branch. In place: `Core.Register` (a machine with arbitrarily many registers) and `Theory.Godel` (the arithmetic packing a register file into one counter — increment is multiplication by a prime, decrement is division, and the zero test is divisibility). Still to build: realizing those three operations as register-machine loops, simulating `Nat.Partrec.Code` with many registers (`prec` and `rfind'` are the hard cases), and packing down to two. Mathlib has no counter-machine model, so none of this can be adapted from it. |
