...
| **2CM universality bridge** | High | The remaining capstone input, being built on the `counter-machine-bridge` branch. Done: the register-machine transfer loops (`Theory.Transfer` — drain, scaled transfer, exact division branching on the remainder), the arithmetic fragments built on them (`Theory.Arith` — variable multiplication, truncated subtraction, comparison answering by exit address, integer square root by bounded search, `Nat.pair` and `Nat.unpair`), the induction `Nat.Primrec f → RegComputable f` (`Theory.Universal.Primrec`), which discharges all seven constructors of `Nat.Primrec` including `prec`, and the universal register machine built on top of it (`Theory.Universal.Machine`). `universal_machine` fixes one program `U` and proves `(∃ t, RunsTo U (loopState (Nat.pair c n) 0) t) ↔ (Code.eval (ofNat Code c) n).Dom` — halting in both directions, the unbounded search over the step bound standing in for `rfind'`. The backward direction needed no case analysis over where `U` might otherwise stop: the step relation is a function, so one infinite path out of the start state is the machine's whole future (`no_runsTo_of_diverges`, `Theory.Trace`). Still to build: packing the register file down to two counters using `Theory.Godel`. Mathlib has no counter-machine model, so none of this can be adapted from it. |
| **Halting problem undecidability** | High | The capstone: Brainfuck halting is undecidable. Both simulation directions are in place, so a Brainfuck decider would yield a 2CM decider, and `universal_machine` now supplies the other end — a fixed register machine whose halting is equivalent to `Nat.Partrec.Code` halting, so a decider for *its* halting would contradict `ComputablePred.halting_problem`. The remaining gap is the middle: packing that machine's register file down to two counters, which is what joins the two halves into a statement about Brainfuck. |
